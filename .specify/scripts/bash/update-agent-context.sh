...
create_new_agent_file() {
    ...
    local substitutions=(
        ...
        "s|\[EXTRACTED FROM ALL PLAN.MD FILES\]|$tech_stack|g"
        "s|\[ACTUAL STRUCTURE FROM PLANS\]|$project_structure|g"
        "s|\[ONLY COMMANDS FOR ACTIVE TECHNOLOGIES\]|$commands|g"
        "s|\[LANGUAGE-SPECIFIC, ONLY FOR LANGUAGES IN USE\]|$language_conventions|g"
        "s|\[LAST 3 FEATURES AND WHAT THEY ADDED\]|$recent_change|g"
    )
